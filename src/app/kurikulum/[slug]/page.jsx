...
const Materi = ({ params }) => {
    ...
    useEffect(() => {
        ...
        const savedStatus = localStorage.getItem('checkedStatus');
        if (savedStatus) {
    ...
    const toggleCheck = (id) => {
        const newStatus = { ...checkedStatus, [id]: !checkedStatus[id] };
                                        ...
                                        <div className="item flex flex-col text-black">
                                            {materiArray.map((materiFiltered) => (
                                                <Link key={materiFiltered.title} href={`/kurikulum/${data?.slug}/${materiFiltered.id}/pembahasan`} className="font-sans bg-black justify-items-start text-xl pl-3 fo">
                                                    <span onClick={(e) => {
                                                        e.preventDefault(); // Mencegah navigasi saat mengklik checkbox
                                                        toggleCheck(materiFiltered.id);
                                                    }}>
                                                        {checkedStatus[materiFiltered.id] ? '✔️' : '⬜'} {/* Simbol checked atau unchecked */}
                                                    </span> 
